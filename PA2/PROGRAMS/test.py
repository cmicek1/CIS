...
def test_normalize():
    print('\nTest normalization.\nUse random input:')
    u = np.random.uniform(-100, 100, (3, 10))
    print('\nu =')
    print(u)

    qs = d.calc_q(u, u)

    n = d.normalize(10, u, qs[0], qs[1])
    ...
    passed = np.all(n >= 0) and np.all(n <= 1)
    assert passed
    print(passed)
...
def test_solve_fcu(tolerance=1e-4):
    print('\nTest dewarping correction.')
    u_star = np.random.uniform(-100, 100, (3, 10))
    du = np.random.uniform(-2, 2, (3,10))

    u = u_star + du
    print('\nRandomly generated observed data (q):')
    print(u)

    print('\nRandomly generated "ground truth" data (q_star):')
    print(u_star)

    qs = d.calc_q(u, u_star)

    u = d.normalize(10, u, qs[0], qs[1])
    u_star = d.normalize(10, u_star, qs[2], qs[3])

    print('\nNormalized observations (u):')
    print(u)
    print('\nNormalized truth (u_star/U):')
    print(u_star)

    print('\nGenerating Bernstein polynomial matrix F...')
    F = d.f_matrix(u, 5)
    print('\nF =')
    print(F)

    print('\nGenerating coefficient matrix C')
    coeffs = d.solve_fcu(F, u_star)
    print('C =')
    print(coeffs)

    print('Is FC == U?')
    passed = np.all(np.abs(F.dot(coeffs) - u_star) <= tolerance)
    assert passed
    print(passed)
